...
def slurm_submit(cmd, p='short', c=1, mem=2, gres=None, J=None, wait_for=[], hold_until_finished=False, log=False, **kwargs):
    '''
    ...
    cmd_sbatch = f'sbatch --wrap "{cmd}" -p {p} -c {c} --mem {mem}g '\
        ...
        f'-o {log_file} '
    cmd_sbatch += ' '.join([f'{"--"+k if len(k)>1 else "-"+k} {v}' for k,v in kwargs.items()])
...
def gpu_array_submit(job_list_file, p='gpu', gres='gpu:rtx2080:1', wait_for=None, log=False, **kwargs):
    return slurm_submit(
...
def cpu_array_submit(job_list_file, J=None, wait_for=None, log=False, **kwargs):
    return slurm_submit(
        cmd = f'eval \`sed -n \${SLURM_ARRAY_TASK_ID}p {job_list_file}\`',
        a = f'1-$(cat {job_list_file} | wc -l)',
        p = 'cpu',
        c = 2,
        mem = 12,
