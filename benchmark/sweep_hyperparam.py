...
def main():
    ...
    if args.benchmarks is not None:
        ...
        for bm in to_run:
            benchmark_list.append([f'--out {args.out}_{bm}', benchmarks[bm].replace('--pdb ','--pdb '+input_path)])
    ...
    if len(benchmark_list) > 0:
        for benchmark in benchmark_list: # pairs of output names and contig specifications
            for i in range(len(arg_combos)):
                arg_combos[i] = benchmark + arg_combos[i]
    else:
        for i in range(len(arg_combos)):
            arg_combos[i] = [f'--out {args.out}'] + arg_combos[i]
    ...
    for icond, arglist in enumerate(arg_combos):
        arglist[0] = arglist[0] + f'_cond{icond}' # tag output prefix with number for each argument combo
