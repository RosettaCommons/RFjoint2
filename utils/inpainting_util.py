...
def preprocess(xyz_t, t1d, DEVICE, masks_1d, ti_dev=None, ti_flip=None, ang_ref=None):
      ...
      alpha, _, alpha_mask = get_torsions(xyz_t.reshape(-1,L,14,3), seq_tmp, ti_dev, ti_flip, ang_ref)
      alpha_mask = torch.logical_and(alpha_mask, ~torch.isnan(alpha[...,0]))
      alpha[torch.isnan(alpha)] = 0.0
      alpha = alpha.reshape(B,-1,L,10,2)
      alpha_mask = alpha_mask.reshape(B,-1,L,10,1)
      chis = torch.cat((alpha, alpha_mask), dim=-1)
      ...
      return (t2d, alpha, alpha_mask, chis, t1d, xyz_t, xyz_prev, state)
