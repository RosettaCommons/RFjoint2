...
class PairStr2Pair(nn.Module):
    def __init__(self, d_pair=128, n_head=4, d_hidden=32, d_rbf=36, d_state=32, p_drop=0.15):
        super(PairStr2Pair, self).__init__()
        
        self.norm_state = nn.LayerNorm(d_state)
        self.emb_rbf = nn.Linear(d_rbf, d_hidden)
        self.norm_rbf = nn.LayerNorm(d_hidden)
        self.proj_rbf = nn.Linear(d_hidden+d_state*2, d_pair)
    ...
    def forward(self, pair, rbf_feat, state):
        ...
        rbf_feat = self.norm_rbf(rbf_feat)
        rbf_feat = torch.cat((rbf_feat, left, right), dim=-1)
...
class Str2Str(nn.Module):
    ...
    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, top_k=128, eps=1e-5):
        B, N, L = msa.shape[:3]
        ...
        R = offset[:,:,1,:] / 100.0 # rotation vector (size = rotation angle)
        #
...
class IterBlock(nn.Module):
    ...
    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, use_checkpoint=False):
        ...
        if use_checkpoint:
            ...
            R, T, state, alpha = checkpoint.checkpoint(create_custom_forward(self.str2str, top_k=128), msa.float(), pair.float(), R_in.float(), T_in.float(), xyz.float(), state.float(), idx)
        else:
            msa = self.msa2msa(msa, pair, rbf_feat, state)
            pair = self.msa2pair(msa, pair)
            pair = self.pair2pair(pair, rbf_feat, state)
            R, T, state, alpha = self.str2str(msa.float(), pair.float(), R_in.float(), T_in.float(), xyz.float(), state.float(), idx, top_k=128) 
...
class IterativeSimulator(nn.Module):
    def __init__(self, n_extra_block=4, n_main_block=12, n_ref_block=4,
        ...
        self.proj_state = nn.Linear(52, SE3_param['l0_out_features'])
    ...
    def forward(self, seq, msa, msa_full, pair, xyz_in, state, alpha, idx, use_checkpoint=False):
        # input:
        ...
        _, xyz = self.compute_allatom_coords(seq, xyz, alpha) 
        if use_checkpoint:
            # Get current structure
            R_in, T_in, state, alpha = checkpoint.checkpoint(create_custom_forward(self.init_str2str),
                                                             msa.float(), pair.float(), R_in.float(), T_in.float(), xyz.float(), state.float(), idx)
        else:
            R_in, T_in, state, alpha = self.init_str2str(msa.float(), pair.float(), R_in.float(), T_in.float(), xyz.float(), state.float(), idx)
        R_s.append(R_in)
        T_s.append(T_in)
        ...
        for i_m in range(self.n_ref_block):
            ...
            xyz = einsum('bnij,bnaj->bnai', R_in, xyz_in) + T_in.unsqueeze(-2)
            _, xyz = self.compute_allatom_coords(seq, xyz, alpha) 
            R_in, T_in, state, alpha = self.str_refiner(msa.float(), pair.float(), R_in.float(), T_in.float(), xyz.float(), state.float(), idx, top_k=128)
            R_s.append(R_in)
            T_s.append(T_in)
            alpha_s.append(alpha)
        ...
        return msa, pair, R_s, T_s, alpha_s, lddt, state
