...
def get_floating_t2d(xyz_t, input_floating_mask, gpu, params=PARAMS, dist_sd = 1):
    """
    This is a function that takes xyz_t, with unmasked floating point residues,
    randomly translates any floating point residues (marked by False in input_floating_mask),
    and then generates t2d features. Dihedral angles for these t2d features are then masked.
    Returns the xyz_t tensor, with translates coordinates, and the t2d features.
    """
    
    B,T,L = xyz_t.shape[:3]
    # make Lx3 tensor of distances, pulled from normal distribution, to translate
    distances = torch.tensor(np.random.normal(0, dist_sd, L), dtype=torch.float32).unsqueeze(dim=-1).repeat(1,3)

    #get random xyz vector for each position
    xyzl = np.asarray([random_three_vector() for i in range(L)])
    xyzl = torch.tensor(xyzl, dtype=torch.float32)
    mask = ~input_floating_mask[0,:].unsqueeze(dim=-1).repeat(1,3)

    #make final Lx3 tensor of x,y,z distances to add to xyzt
    translate = distances*xyzl*mask
    translate = translate.to(gpu, non_blocking=True)
    #expand to 1x1xLx1x3
    translate = translate.unsqueeze(dim=0).unsqueeze(dim=0).unsqueeze(dim=-2)
    translate = translate.repeat(B,T,1,14,1)

    xyz_translate = torch.clone(xyz_t)
    xyz_translate = torch.add(xyz_translate, translate)
    
    #make t2d features from these translated residues
    t2d = xyz_to_t2d(xyz_translate, params)
    
    #mask angles associated with these residues
    t2d[:,:,~input_floating_mask[0,:],:,37:] = 0
    t2d[:,:,:,~input_floating_mask[0,:],37:] = 0
    
    #now mask coordinates of floating points in xyz_t, so that these are not given as the initial guess (where backbone orientation is implicitly given)
    xyz_t_out = torch.clone(xyz_translate)
    xyz_t_out[:,:,~input_floating_mask[0,:],:,:] = 0.0

    return t2d, xyz_t_out
...
def get_init_xyz(xyz_t):
    ...
    B, T, L = xyz_t.shape[:3]
    init = INIT_CRDS.to(xyz_t.device).reshape(1,1,1,14,3).repeat(B,T,L,1,1)
    ...
    #
    xyz = torch.where(mask.view(B, T, L, 1, 1), init, xyz_t).contiguous()
    return xyz
