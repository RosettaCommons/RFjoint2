...
class TemplatePairStack(nn.Module):
    def __init__(self, n_block=2, d_templ=64, n_head=4, d_hidden=32, p_drop=0.25):
        ...
        proc_s = [PairStr2Pair(d_pair=d_templ, n_head=n_head, d_hidden=d_hidden, d_state=52, p_drop=p_drop) for i in range(n_block)]
        self.block = nn.ModuleList(proc_s)
        self.norm = nn.LayerNorm(d_templ)
    def forward(self, templ, xyz_t, t1d, use_checkpoint=False):
        B, T, L = templ.shape[:3]
        ...
        for i_block in range(self.n_block):
            if use_checkpoint:
                templ = checkpoint.checkpoint(create_custom_forward(self.block[i_block]), templ, rbf_feat, t1d)
            else:
                templ = self.block[i_block](templ, rbf_feat, t1d)
...
class Templ_emb(nn.Module):
    ...
    #   - confidence (1)
    #   - 7 torsion angles + 3 additional angles for CB and CG (30; cos, sin, mask)
    #   
    def __init__(self, d_t1d=21+1+30, d_t2d=43+1, d_pair=128, n_block=2, d_templ=64,
    ...
    def forward(self, t1d, t2d, xyz_t, pair, use_checkpoint=False):
        ...
        templ = self.templ_stack(templ, xyz_t, t1d, use_checkpoint=use_checkpoint) # template templure
...
class Recycling(nn.Module):
    def __init__(self, d_msa=256, d_pair=128):
        ...
        self.proj_dist = nn.Linear(36*2+104, d_pair)
    ...
    def forward(self, seq, msa, pair, xyz, state):
        ...
        # get Tip atoms
        Tip = get_tips(xyz, seq)
        
        dist = rbf(torch.cdist(Cb, Cb))
        dist_tip = rbf(torch.cdist(Tip, Tip))
        dist = torch.cat((dist, dist_tip, left, right), dim=-1)
